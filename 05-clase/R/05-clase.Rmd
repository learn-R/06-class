...
Profundizaremos algunos aspectos de la manipulación de datos, en particular, agrupar datos según alguna(s) columna(s) o filas en particular,  pivotando sets de datos, y uniendo y/o separando datos. 
...
datos <- datos %>%
  rowwise() %>% 
  mutate(ing_tot = sum(ss_t, svar_t, reg_t)) 
```
...
datos %>% 
  ...
  summarise(media = mean(ss_t)) #Creamos una columna llamada media, calculando la media ingresos con la función `mean`
```
...
1. Calculen cuantas personas tienen contrato definido e indefinido
2. Calcular la composición sexual del contrato definido
3. Calcular índice promedio entre las variables que terminan en *t* (no olvide ocupar los recursos de select)
...
1. Cada variable tiene que estar su propia columna `pivot_*`

2. Cada observación tiene que estar en su propia fila `pivot_*`

3. Cada valor tiene que estar en su propia celda. `separate` y `unite`
...
Como ya se señaló, `pivot_longer()`nos permitirá pivotear los datos de manera tal que estos se transformen a un tabulado en formato **long**. Ello implica que el tabulado tendrá un mayor número de observaciones, reduciendo su cantidad de columnas.

![](https://d33wubrfki0l68.cloudfront.net/3aea19108d39606bbe49981acda07696c0c7fcd8/2de65/images/tidy-9.png)

En este caso, pivotearemos todas las columnas salvo `sector`, lo cual significa que tales columnas se subsumirán en una sola (en este caso, llamada "variable"), y sus respectivos valores en otra (en este caso, llamada "value"). Luego, la unidad de análisis pasará, de ser sujetos, a ser las variables. Para ello usamos el siguiente código:

```{r pivot-longer, echo=TRUE}
datos %>% #Creamos un nuevo objeto a partir de datos
  select(-ing_tot, -ends_with("_t")) %>%  #Des-seleccionamos la variable ing_tot, pues es de class numeric
...
```{r}
long <- datos %>% 
  select(-ing_tot, -ends_with("_t")) %>%
  pivot_longer(cols=-c(1, id),
               names_to = "variable",
               values_to = "value") 
```
...
Asimismo, las tablas largas (long) pueden ser transformadas a tablas anchas (wide) con la función `pivot_wider()`, lo cual implica que nuestra tabla reducirá su cantidad de filas y aumentará su número de columnas.



![](https://d33wubrfki0l68.cloudfront.net/8350f0dda414629b9d6c354f87acf5c5f722be43/bcb84/images/tidy-8.png)

Para ello, sólo debemos especificar desde dónde extraeremos los nombres y los valores asociados a las nuevas columnas: 
...
La función `separate()` de `tidyr` nos permite convertir un character individual de una columna en múltiples columnas.

![](https://d33wubrfki0l68.cloudfront.net/f6fca537e77896868fedcd85d9d01031930d76c9/637d9/images/tidy-17.png)

En este caso, crearemos una nueva variable `sobre_prom`, que nos indicará si los ingresos de los sujetos están por sobre o por debajo de la media de ingresos. Luego, separaremos la primera y la segunda palabra (caracteres) de cada unos de los valores en dos columnas distintas.
...
También podemos unir caracteres de columnas distintas, con la función `unite()` de `tidyr`.


![](https://d33wubrfki0l68.cloudfront.net/3d98d3ba019fed3f9ee328284568d4508e479ef8/0b3e6/images/tidy-18.png)

En este caso, volveremos a unir las columnas ¿Sobre o bajo? y Media en una sola, llamada sobre_prom. Por defecto, `unite()` une los caracteres con guiones bajos (_). Podemos especificar un nuevo separador con el argumento `sep`. En este caso, el separador será un espacio (" ")
...
4. Define qué procedimiento quieres hacer
  ...
  - ¿combinar, pegar filas, pegar columnas?
