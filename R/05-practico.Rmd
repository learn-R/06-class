...
datos <- readRDS("input/data/ESI2020.rds")
```
...
long <- datos %>% #Creamos un nuevo objeto a partir de datos
  ...
  pivot_longer(cols=-1, #Pivoteamos a lo largo todas las columnas salvo la primera (sector) 
...
Podemos darnos cuenta de que ahora nuestro set de datos sólo contiene 3 columnas, pero `r format( nrow(long), decimal.mark="," , big.mark = ".")` filas. Podemos darnos cuenta que ahora, cada cuatro columnas, tenemos a un sujeto de la muestra. 
...
wide <- long %>% #Creamos un nuevo objeto a partir de los datos de long 
  pivot_wider(names_from = "variable", #Pivoteamos, tomando el nombre de las columnas desde la columna "variable"
              values_from = "value") %>% #Y los valores desde la columna "value"
  unnest()  #Desanidamos los valores  

```

Los datos se "ensancharon", pasando a tener `r format( nrow(wide), decimal.mark="," , big.mark = ".")` filas y `r format( ncols(wide), decimal.mark="," , big.mark = ".")` columnas. 
...
```{r merge, echo = T}
esi2020_m <- readRDS("input/data/ESI2020m.rds") #Cargamos los datos que deseamos unir
